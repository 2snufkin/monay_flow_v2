...
from src.config.database_config import get_sqlite_connection, get_mongo_database, get_mongo_collection
...
class MongoDBE2ETester:
    ...
    def __init__(self):
        ...
        self.test_results = []
        self.test_schema_name = f"MongoDB E2E Test {datetime.now().strftime('%Y%m%d_%H%M%S')}"
        self.test_excel_file = Path("test_data_with_emails.xlsx")
        self.mongodb_collection_name = "test25"
        
        # Test data for Excel file
        self.test_data = {
            'First Name': ['John', 'Jane', 'Bob', 'Alice', 'Charlie', 'Diana', 'Eve', 'Frank', 'Grace', 'Henry'],
    ...
    def create_test_excel_file(self) -> bool:
        ...
        try:
            ...
            self.log(f"📧 Emails: {len([email for email in df['Email'] if '@' in email])} valid emails")
    ...
    def test_mongodb_connection(self) -> bool:
        ...
        try:
            ...
            self.log("💡 Make sure your MongoDB Atlas credentials are correct in .env file", "WARNING")
            return False
    ...
    def create_schema_with_email_validation(self) -> Optional[str]:
        ...
        try:
            ...
            self.log(f"   🔍 Duplicate detection: {schema_def.duplicate_detection_columns}")
    ...
    def process_excel_to_mongodb(self, schema_id: str) -> bool:
        ...
        try:
            ...
            self.log(f"📊 File info: {file_info.total_rows} rows, {file_info.total_columns} columns")
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    document = {
                        ...
                        'email': row['Email'],
                        'phone': row['Phone'],
                        'purchase_date': row['Purchase Date'].isoformat() if pd.notna(row['Purchase Date']) else None,
            ...
            self.log(f"📊 Processing complete: {inserted_count} inserted, {skipped_count} skipped")
        ...
        try:
            ...
            if duplicate_check:
                ...
                try:
                    ...
                    self.log("❌ Duplicate email was inserted - index not working", "ERROR")
                    return False
                except Exception as e:
                    if "duplicate key error" in str(e).lower():
                        self.log("✅ Duplicate prevention working correctly")
                    else:
                        self.log(f"⚠️ Unexpected error on duplicate insert: {e}", "WARNING")
            
            self.log(f"✅ Data verification complete: {len(found_emails)}/{len(test_emails)} emails found")
    ...
    def cleanup_test_data(self):
        ...
        try:
            ...
            self.log(f"✅ Deleted {result.deleted_count} documents from MongoDB collection '{self.mongodb_collection_name}'")
