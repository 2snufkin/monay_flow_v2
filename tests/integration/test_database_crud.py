...
def test_file_processing_history_crud():
    ...
    cursor.execute("SELECT * FROM file_processing_history WHERE file_name = ?", ("test_excel.xlsx",))
    row = cursor.fetchone()
    if row:
        print(f"   ✅ Retrieved record: {row['file_name']} - Success: {row['success_count']}")
    ...
    cursor.execute("DELETE FROM file_processing_history WHERE file_name = ?", ("test_excel.xlsx",))
...
def test_audit_log_crud():
    ...
    row = cursor.fetchone()
    if row:
        print(f"   ✅ Retrieved record: {row['id']} - Operation: {row['operation_type']}")
...
def test_data_quality_issues_crud():
    ...
    cursor.execute("SELECT * FROM data_quality_issues WHERE batch_id = ? AND row_number = ?", ("test_batch_123", 15))
    row = cursor.fetchone()
    if row:
    ...
    cursor.execute("DELETE FROM data_quality_issues WHERE batch_id = ? AND row_number = ?", ("test_batch_123", 15))
    deleted_count = cursor.rowcount
    conn.commit()
    print(f"   ✅ Deleted {deleted_count} record(s)")
...
def test_schema_analytics_crud():
    ...
    cursor.execute("SELECT * FROM schema_analytics WHERE schema_id = ?", ("test_schema_123",))
    row = cursor.fetchone()
    if row:
    ...
    cursor.execute("DELETE FROM schema_analytics WHERE schema_id = ?", ("test_schema_123",))
    deleted_count = cursor.rowcount
    conn.commit()
    print(f"   ✅ Deleted {deleted_count} record(s)")
